document.addEventListener("DOMContentLoaded", function () {
  ...
  function displayCategories(categories) {
    ...
    categories.forEach((cat) => {
      ...
      const catName =
        cat.category || cat.name || cat.category_name || cat.title;
  ...
  function displayPlants(plants) {
    ...
    plants.forEach((plant) => {
      ...
      card.className =
        "p-4 bg-white rounded-lg shadow flex flex-col justify-between";
  ...
  function renderCart() {
    ...
    cart.forEach((item) => {
      ...
      row.className =
        "p-2 bg-[#f0fdf4] rounded flex items-center justify-between";
      row.innerHTML = `
        <div>
      ...
      row
        ...
        .addEventListener("click", () => removeFromCartById(item.id));
